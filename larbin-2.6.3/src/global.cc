...
int global::IPUrl = 0;
/*modified by shenyj*/
std::vector<std::string> global::lsection;
...
global::global (int argc, char *argv[]) {
  ...
  proxyAddr = NULL;
  domains = NULL;
  // FIFOs ,modified by shenyj for spectial fifo,fifowait file
  std::string conf = configFile;
  std::string::size_type pos1 = conf.find_last_of("/");
  std::string::size_type pos2 = conf.find_last_of(".");
  std::string busi = conf.substr(pos1+1,pos2-pos1-1);
  std::string fifo = fifoFile;
  std::string fifowait = fifoFileWait;
  fifo += "_"; fifo += busi;
  fifowait += "_"; fifowait += busi;
  /*
  URLsDisk = new PersistentFifo(reload, fifoFile);
  URLsDiskWait = new PersistentFifo(reload, fifoFileWait);*/
  URLsDisk = new PersistentFifo(reload, (char*)fifo.c_str());
  URLsDiskWait = new PersistentFifo(reload, (char*)fifowait.c_str());
...
void global::parseFile (char *file) {
  ...
  while (tok != NULL) {
	if (!strcasecmp(tok, "UserAgent")) {
	  ...
	  externalLinks = false;
	}else if (!strcasecmp(tok,"section")){
		 tok = nextToken(&posParse);
		 std::string section = tok;
		 std::string::size_type bpos,pos;
		 bpos = pos = 0;
		 while(bpos < section.size() && (pos = section.find(",",bpos)) != std::string::npos){
		 	lsection.push_back(section.substr(bpos,pos-bpos));
			bpos = pos + 1;
		 }
		 std::copy(lsection.begin(),lsection.end(),std::ostream_iterator<std::string>(std::cout, "\n"));
	}

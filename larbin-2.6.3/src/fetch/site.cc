...
void NamedSite::putGenericUrl(url *u, int limit, bool prio) {
  if (nburls > maxUrlsBySite-limit) {
    ...
    nburls++;
	//dnsState constructor set waitDns, when resultRebots() set doneDns
    if (dnsState == waitDns
...
void NamedSite::newQuery () {
  ...
  } else {cout<<"[NamedSite::newQuery ()],adns_submit,name="<<name<<endl;
    // submit an adns query
    global::nbDnsCalls++;
    adns_query quer = NULL;
	//query parameter, name=startUtl ,NamedSIte=this .by shenyj
  ...
  if (ans->status == adns_s_prohibitedcname) {
    ...
    if (ans->status != adns_s_ok) {
      ...
      memcpy (&addr,
              ...
              sizeof (struct in_addr));
	  cout<<"NamedSite::dnsAns(),copy ans addr to namedSite addr,addr="<<
	  	inet_ntoa(addr)<< endl;
      // Get the robots.txt
      dnsOK();
    }
...
//This conn only get,not set ,no lose? by shenyj
  Connexion *conn = global::freeConns->get();
  //transfer dns(www) to ip(socket) by shenyj
  ...
  if (res != emptyC) {
    ...
    conn->pos = 0;
    conn->err = success;
    conn->state = res;cout<<"NamedSite::dnsOK ,request="<<
		 conn->request.getString()<<endl;
  } else {
...
void NamedSite::transfer (url *u) {
  if (testRobots(u->getFile())) {
    ...
    }cout<<"transfer,global::IPSiteList[ipHash].putUrl(u),file="<<u->getFile()
    <<","<<u->getHost()<<endl;
    global::IPSiteList[ipHash].putUrl(u);
  } else {cout<<"transfer,forgetUrl()"<<endl;
    forgetUrl(u, forbiddenRobots);
  }
}
...
void IPSite::putUrl (url *u) {
  ...
  if (!isInFifo) {
    ...
    if (lastAccess + global::waitDuration <= global::now
      ...
      printf("[IPSite:putUrl],host=[%s],file=[%s],port=[%d]\n",u->getHost(),u->getFile(),u->getPort());
  ...
  if (tab.isEmpty()) {
    ...
    if (next_call > global::now) {
      ...
      if (res != emptyC) {
        ...
        if (tab.isEmpty()) {
          isInFifo = false;
        } else {cout<<"[IPSite::fetch ()]global::okSites->put,request="
        	<<conn->request.getString()<<endl;
		//why okSites put? alreadey get a site? added by shenyj
          global::okSites->put(this);
        }
        return 0;
      } else {

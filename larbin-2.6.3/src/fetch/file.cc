...
#ifdef SHENYJ
cout<<"[robots:parse()],isError="<<isError<<","<<answerCode<<endl;
#endif
  //answerCode = true where http call return  2xx
...
#ifdef SHENYJ
cout<<"parseRobots,posParse="<<posParse<<endl;
#endif
  robotsOK();
#ifndef NOSTATS
  bool goodfile = true;
...
void html::manageUrl (url *nouv, bool isRedir) {
 /*modified by shenyj for 
	nouv->getHost():http://zhengwu.beijing.gov.cn,
	this->here->getHost() :www.beijing.gov.cn
	2014/12/14 10:21 modified org by sdzc.conf
 **/
  if (nouv->isValid()
...
/*
    std::string urlhost = nouv->getHost();//http://zhengwu.beijing.gov.cn
  	std::string herehost = this->here->getHost();//www.beijing.gov.cn
  	herehost = herehost.substr(4,herehost.size()-4);
	//replace !strcmp(nouv->getHost(), this->here->getHost())
	cout<<"herehost="<<herehost<<endl;
	bool flag = false;
	if ((urlhost.find(herehost) != std::string::npos)||global::externalLinks || isRedir){
		 flag = true;
	}
...
#ifdef SHENYJ
cout<<"[html]:inputHeaders,posParse="<<posParse<<
	",state="<<state<<",size="<<size<<endl;
#endif
  pos += size;
  buffer[pos] = 0;
  char *posn;
...
int html::endInput () {
#ifdef SHENYJ
cout<<"[html:endInput()],state="<<state<<endl;
#endif
  if (state <= HEADERS) {
    errno = earlyStop;
    return 1;
...
/* parse an html page */
void html::parseHtml () {
  cout<<"[html]parseHtml begin..."<<endl;
  while ((posParse=strchr(posParse, '<')) != NULL) {
  	#ifdef SHENYJ
  	cout<<"while,posParse="<<posParse<<endl;
	#endif
    if (posParse[1] == '!') {
...
void html::parseTag () {
  ...
  for (;;) {
	...
	string tmp  = posParse;
	if(tmp.size() > 50)
	cout<<"[parseTag]:param="<<param<<",action="<<action<<","<<i 
		<<",param[i]="<<param[i]<<"|"<< tmp.substr(0,40)<<endl;
	#endif
	if (param[i] == 0) {
      parseContent(action);
      return;
...
void html::parseContent (int action) {
  ...
  string tmp  = posParse;
	if(tmp.size() > 50)
		cout<<"[parseContent] begin "<<tmp.substr(0,40)<<endl;
#endif
  posParse++;
  while (*posParse==' ' || *posParse=='=') posParse++;
  if (*posParse=='\"' || *posParse=='\'') posParse++;
  skipSpace();//added by shenyj for skip space ,"  /sites/ql/c19/d_20141124162109860120
  area = posParse;
  ...
  if (posParse == buffer + pos) {
    ...
    *posParse = 0;
    ...
    case LINK:
      ...
      cout<<"Find LINK:"<<area<<endl;
	  if (global::lsection.size() == 1 && global::lsection[0] == "all"){
	  	printf("[html:parseContent()]add a new url[%s],[%s],depth=[%d]\n",area,base->getHost(),here->getDepth()-1);
		manageUrl(new url(area, here->getDepth()-1, base), false);
	  }
	  else {/*
	  	bool flag  = false;
	  	for(int i = 0;i < global::lsection.size();++i){
	  		std::string news = area;
			if (news.find(global::lsection[i]) != string::npos)
			{
				flag = true;
				break;
			}
	 	...
	 	//modified by shenyj  for find_if whith pred 2014/12/11 13:35
	 	std::string news = area;
	 	typedef std::vector<std::string>::const_iterator IV;
		IV p = std::find_if(global::lsection.begin(),global::lsection.end(),section(news));
		if (p != global::lsection.end()){
			printf("[html:parseContent()]add a new url[%s],[%s],depth=[%d]\n",area,base->getHost(),here->getDepth()-1);
			manageUrl(new url(area, here->getDepth()-1, base), false);
		}
